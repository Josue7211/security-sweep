---
name: security-check
description: Surgical security scan on recently changed files only. Same 100% quality as /security-sweep (confidence scores, exploit PoCs, attack chains, defense depth) but scoped to recent changes. 5 Opus agents run in parallel on the diff. Use after features, bug fixes, or any code change. Use /security-sweep for full codebase audits.
---
...
## Step 1: Launch 5 agents (parallel, Opus)

All agents get the TARGET_FILES list. They read ONLY those files + their direct dependencies (e.g., if auth.rs changed, also read server.rs for UserSession).
...
- New format!() with user input? SQL injection? PostgREST injection?
- New unsafe HTML rendering? DOMPurify applied?
- New external requests? SSRF protected?
- New file operations? Path traversal blocked?

### Agent 3: Crypto + Secrets + Config
If crypto.rs, secrets.rs, auth.rs, tauri.conf.json, or any config changed:
- Key handling correct? Zeroized?
- CSP still tight? New connect-src needed?
- Secrets exposed? New VITE_ env vars?
...
- Silent failures (let _ = on critical ops)?
